...
int numReadings =
    0; // python threads maximum reached after about a dozen readings
...
void ShClearMatrix(void) {
  ...
  PyRun_SimpleString("from sense_emu import SenseHat\n"
                     "sense=SenseHat()\n"
                     "sense.clear()\n");
#else
...
lps25hData_s ShGetLPS25HData(void) {
  ...
  // Run one-shot measurement (temperature and humidity). The set bit will be
  // reset by the sensor itself after execution (self-clearing bit)
...
ht221sData_s ShGetHT221SData(void) {
  ...
  int16_t T_OUT;
  uint8_t h0_out_l, h0_out_h, h1_out_l, h1_out_h, h0_rh_x2, h1_rh_x2, h_t_out_l,
      h_t_out_h;
